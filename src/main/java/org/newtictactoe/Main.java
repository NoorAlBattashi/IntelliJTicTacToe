...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        if (choose == 1) {
            ...
            if (player2Name.equals("Bot")) {
                ...
                player2 = new Bot(player2Name, player2Symbol);
            } else {
                player1 = new Player(player1Name, player1Symbol);
            ...
            System.out.println("2-Player1 vs Bot");
            System.out.print("Select Here: ");
            int gameType = intScanner();
            ...
            if (gameType == 1) {
                ...
                // player1 details
                System.out.println("Bot Symbol is : X");
                System.out.print("Player 1 write your name: ");
                String player1Name = stringScanner();
                System.out.print(player1Name + ", write your Symbol: ");
                String player1Symbol = stringScanner();
                ...
                if (player1Symbol.equals("X")) {
                    System.out.println("Bot Symbol is : X");
                    System.out.print("Choose another Symbol here: ");
                    player1Symbol = stringScanner();
                }

                //add players
                Player player1 = new Player(player1Name, player1Symbol);
                Player Bot = new Bot("Bot", "X");

                // start new game
                startGame(validTurnsCounter, isThereAWinner, isDraw, player1, Bot, PlayerOneTurn,
                        PlayerTwoTurn);
    ...
    public static void startGame(int validTurnsCounter, boolean isThereAWinner, boolean isDraw, Player player1,
                                 Player player2, boolean playerOneTurn, boolean playerTwoTurn) {
        ...
        while (validTurnsCounter < 9 && isThereAWinner == false && isDraw == false) {
            ...
            if (playerchoice == 0) {
                ...
                manager.storeTheGame("ticTacToeArr", boardOfGame.getBoard());
                ...
                if (playerOneTurn == true && playerTwoTurn == false && isThereAWinner == false && isDraw == false) {
                    ...
                    int player1Move = player1.makeMove(boardOfGame.getBoard(), player2.getSymbol());

                    // check the selection position of player1 and add value
                    if (playerOneTurn == true && playerTwoTurn == false) {
                        playerOneTurn = boardOfGame.checkMove(player1Move, boardOfGame.getBoard(), player1.getSymbol(), player2.getSymbol(), playerOneTurn);
                    ...
                    // check winner
                    if (!isThereAWinner) {
                        isThereAWinner = checkWinner(boardOfGame.getBoard(), player1);
                    }

                    // check Draw
                    if (!isThereAWinner) {
                        isDraw = checkDraw(boardOfGame.getBoard());
                    }
                }
                // player 2 turn
                if (playerOneTurn == false && playerTwoTurn == true && isThereAWinner == false && isDraw == false) {
                    System.out.println();
                    if (player2.getplayerName().equals("Bot")) {
                        System.out.println(player2.getplayerName() + ", Make The move");
                    } else {
                        System.out.print(player2.getplayerName() + ", Make a move, Enter position: ");
                    }
                    int player2Move = player2.makeMove(boardOfGame.getBoard(), player1.getSymbol());

                    // check the selection position of player2 and add value
                    if (playerTwoTurn == true && playerOneTurn == false) {
                        playerTwoTurn = boardOfGame.checkMove(player2Move, boardOfGame.getBoard(), player2.getSymbol(), player1.getSymbol(), playerTwoTurn);
                    ...
                    // check winner
                    if (!isThereAWinner) {
                        isThereAWinner = checkWinner(boardOfGame.getBoard(), player2);
                    }
                    // check Draw
                    if (!isThereAWinner) {
                        isDraw = checkDraw(boardOfGame.getBoard());
